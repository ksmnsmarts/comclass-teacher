...
export class ComclassSlideViewComponent implements OnInit {
    ...
    eventBusListeners() {
        ...
        this.eventBusService.on(
            ...
            async (data) => {
                this.scrollRatio =
                    this.thumbArray[this.currentPageNum - 1].width / data.coverWidth;
                this.thumbWindowSize = {
                    width:
                        this.thumbArray[this.currentPageNum - 1].width * data.ratio.w +
                        'px',
                    height:
                        this.thumbArray[this.currentPageNum - 1].height * data.ratio.h +
                        'px',
                };
    ...
    drawThumbRx(data) {
        ...
        if (this.currentDocId == data.docId) {
            ...
            const thumbCanvas =
                this.thumbCanvasRef.toArray()[data.pageNum - 1].nativeElement;
            const thumbScale = this.thumbArray[this.currentPageNum - 1].scale;
