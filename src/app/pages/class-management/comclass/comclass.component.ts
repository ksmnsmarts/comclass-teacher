...
export class ComclassComponent implements OnInit {
    ...
    async openFile(fileInput, sourceType) {
        ...
        this.classService.uploadDocument(formData, this.classId).subscribe((result: any) => {
    ...
    async updateDocuments() {
        ...
        const result: any = await this.classService.getDocumentsInfo(data).toPromise();
        ...
        for (let i = 0; i < result.docResult.length; i++) {
            ...
            if (pdfArrayVar[i]?.updatedAt !== updatedTime) {
                ...
                try {
                    console.log('aaa')
                    // PDF File 정보 요청
                    const res = await this.classService.getPdfFile(data).toPromise()
                    console.log(res)
                    // Array buffer로 변환
                    const file = await this.fileService.pdfReadFile(res, 'pdf');
